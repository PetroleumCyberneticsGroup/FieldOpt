...
TEST_F(WellConstraintProjectionsTests, LU_factorisation){
  ...
  QList<Eigen::Vector3d>
      solution_vectors = WellConstraintProjections::non_inv_solution(A,b);

  for(int i=0; i<solution_vectors.length(); i++){
    std::cout << "solution vector with length "
              << sqrt(solution_vectors.at(i)(0)*solution_vectors.at(i)(0)
...
TEST_F(WellConstraintProjectionsTests, eigen_eigensolver_test){
  ...
  cout << "Here is a random symmetric 5x5 matrix, A:"
       << endl << A << endl << endl;
  ...
  cout << "The matrix of eigenvectors, V, is:"
       << endl << es.eigenvectors() << endl << endl;

  // -------------------------------------------------------------
  double lambda = es.eigenvalues()[1];
  cout << "Consider the second eigenvalue, lambda = "
       << lambda << endl;
  ...
  cout << "Finally, V * D * V^(-1) = "
       << endl << V * D * V.inverse() << endl;

  // -------------------------------------------------------------
  cout << "checking eigenvector dot products, should all be zero" << endl;

  cout <<"v1,v2 = "
       << es.eigenvectors().col(0).transpose()*es.eigenvectors().col(1) << endl;

  cout <<"v1,v3 = "
       << es.eigenvectors().col(0).transpose()*es.eigenvectors().col(2) << endl;

  cout <<"v3,v2 = "
       << es.eigenvectors().col(2).transpose()*es.eigenvectors().col(1) << endl;
}
...
TEST_F(WellConstraintProjectionsTests, eigen_kkt_solver){
  ...
  std::cout << "shortest distance"
            << WellConstraintProjections::shortest_distance(newcoords)
            << std::endl;
  ...
  // ---------------------------------------------------------------
  QList<Eigen::Vector3d>
      moved_points = WellConstraintProjections::interwell_constraint_projection(newcoords, d);
...
TEST_F(WellConstraintProjectionsTests, interwell_multiple_wells_test){
  ...
  // ---------------------------------------------------------------
  QList<QList<Eigen::Vector3d>> all_wells_moved =
      WellConstraintProjections::interwell_constraint_multiple_wells(all_wells, 4, 10e-4);

  QList<QList<Eigen::Vector3d>> all_wells_length_moved =
      WellConstraintProjections::well_length_constraint_multiple_wells(all_wells,10, 5,10e-8);

  QList<QList<Eigen::Vector3d>> all_wells_both_proj =
      WellConstraintProjections::both_constraints_multiple_wells(all_wells, 4, 10e-4, 10, 5, 10e-8);
...
TEST_F(WellConstraintProjectionsTests, threepoint_interwell_error_test){
  ...
  // ---------------------------------------------------------------
  moved_wells =
      WellConstraintProjections::interwell_constraint_projection(wells, d);
