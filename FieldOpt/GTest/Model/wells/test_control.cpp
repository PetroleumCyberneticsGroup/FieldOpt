...
TEST_F(ControlTest, VariableContainerConsistencyAfterCreation) {
    ...
    EXPECT_EQ(1, variable_container_->RealVariableSize()); // BHP for PROD is the only control that's set to variable.
...
TEST_F(ControlTest, VariableContainerConsistencyAfterModification) {
    ...
    EXPECT_FLOAT_EQ(500, all_controls_.first()->bhp());

    // Changing value on control object
    all_controls_.first()->setBhp(100); // should change the first real variable in the variable container
