...
// =========================================================
Model::Wells::Wellbore::Completions::Completion
*WellBlock::GetCompletion() {
  if (completion_ == 0) {
    throw Model::Wells::
    CompletionNotDefinedForWellBlockException(i_->value(),
                                              j_->value(),
                                              k_->value());
  }
  return completion_;
...
bool WellBlock::HasPerforation() {
  ...
  if (completion_ != 0 && completion_->type() ==
      Completions::Completion::CompletionType::Perforation) {
...
Completions::Perforation *WellBlock::GetPerforation() {
  ...
  throw PerforationNotDefinedForWellBlockException(i_->value(),
                                                   j_->value(),
                                                   k_->value());
}
}
