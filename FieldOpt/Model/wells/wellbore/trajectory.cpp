...
void Trajectory::calculateDirectionOfPenetration() {
  ...
  // -------------------------------------------------------
  // Assuming that the well is vertical if it only has one block
  if (well_blocks_->size() == 1) {
    well_blocks_->first()->
        setDirectionOfPenetration(WellBlock::DirectionOfPenetration::Z);
    return;
  }
  ...
  for (int i = 0; i < well_blocks_->size()-1; ++i) {
    ...
    if (std::abs(well_blocks_->at(i)->i() - well_blocks_->at(i+1)->i()) == 1 &&
      ...
      well_blocks_->at(i)->
          setDirectionOfPenetration(WellBlock::DirectionOfPenetration::W);
    }
  }
