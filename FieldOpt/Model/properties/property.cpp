...
Property::SplineEnd Model::Properties::Property
::get_spline_end(const QString prop_name) const {

    if (prop_name.split("#").length() != 4){
        throw std::runtime_error(
            "Invalid SplinePoint format "
                + prop_name.toStdString()
    ...
    if (QString::compare("heel", endstr) == 0)
        ...
        throw std::runtime_error(
            "Invalid SplinePoint name format "
                + prop_name.toStdString()
                + ", unable to extract heel/toe info.");
    }
}
...
::get_prop_type_name(const QString prop_name) const {
...
int Property::get_prop_index(const QString prop_name) const {
    ...
    if (cast_ok)
        ...
        throw std::runtime_error(
            "Unable to extract index from property name "
                + prop_name.toStdString());
    }
}
...
Property::get_prop_coord(const QString prop_name) const {
    ...
    if (QString::compare("x", cstr) == 0) return x;
        ...
        throw std::runtime_error(
            "Unable to extract coordinate from property name "
                + prop_name.toStdString());
    }
}
