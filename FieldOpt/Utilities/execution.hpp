 ...
 * \param verbose Whether or not the output of the command should
 * be displayed in the terminal.
 * \return The output from the command.
 */
inline QString Exec(QString directory, QStringList commands,
                    bool verbose=false)
{
                            ...
                            std::vector<int> verb_vector)
{
  if (!Utilities::FileHandling::FileExists(script_path)) {
    Utilities::FileHandling::ThrowRuntimeError(
        "File not found: " + script_path.toStdString());
  }
...
namespace helpers {
/*!
 * Create child-process and execute the given script with the arguments given in it.
...
/*!
 * @brief ExecShellScriptTimeout executes a shell script with the given
 * set of parameters, and terminates the process after a set time has
 * passed if it has not returned by then.
 *
 * \todo In this function, in the else block, we can also, at a later
 * stage, monitor the output files of a simulation and use them to
 * decide whether we should abort.
 *
 * @param script_path Absolute path to the shell script.
 * @param args Arguments to be passed to the script.
 * @param timeout Seconds before the execution will be terminated.
 * @return True if script returned _before_ the timeout, otherwise false.
 */
inline bool ExecShellScriptTimeout(QString script_path, QStringList args,
                                   int timeout)
{
  if (!Utilities::FileHandling::FileExists(script_path))
