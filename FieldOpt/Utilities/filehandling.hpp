...
/*!
 * \brief FileExists Checks whether or not a file exists at the
 * specified path.
 * \param file_path Path to a file that may or may not exist.
 * \return True if a file exists at the specified path, otherwise
 * false.
 */
...
/*!
 * \brief GetFileSize Returns the size of the file (in bytes) given
 * by the specified path. It checks whether the file exists, and
 * throws error if it doesn't.
 * \param file_path Path to a file that may or may not exist.
 * \return file_sz File size in bytes (type int)
 */
inline int GetFileSize(std::string file_path)
{
    uintmax_t f_sz;
    boost::filesystem::file_status fs = boost::filesystem::status(file_path);
    if (exists(fs))
...
/*!
 * \brief DirectoryExists Checks whether or not a folder exists
 * at the specified path.
 * \param folder_path Path to a folder that may or may not exist.
 * \return True if a folder exists at the specified path, otherwise
 * false.
 */
...
/*!
 * \brief ParentDirectoryExists Checks whether a specified file's
...
/*!
 * \brief ReadFileToStringList Reads the contents of a file and
 * stores it as a string list where each element is a line
 * in the file.
...
inline void DeleteFile(QString path)
{
    if (FileExists(path)) {
        ...
        file.remove();
            ...
            "File not found: " + path.toStdString());
}

/*!
 * \brief CreateDirectory Create a new drectory with the
 * specified path.
...
/*!
 * \brief CopyDirectory Copy a directory and it's
 * contents to a new destination.
 *
 * Note that this is not a recursive function: It will copy
 * files in the root of the directory, and _create_ any
 * subdirectories found, but it will not copy the contents
 * of subdirectories.
...
inline void CopyDirectory(QString origin, QString destination)
{
    ...
    if (!DirectoryExists(destination))
        throw std::runtime_error(
            "Can't find destination (parent) directory for copying: "
                + destination.toStdString());

    QDir original(origin);
    QFileInfoList entries = original.entryInfoList(
        QDir::AllEntries | QDir::NoDotAndDotDot, QDir::DirsLast);

    for (auto entry : entries) {
        if (entry.isFile() && !entry.isDir())
            CopyFile(entry.absoluteFilePath(), destination
                + "/" + entry.fileName(), true);
            //std::cout << "FILE: " << QString().toStdString() << std::endl;
        else if (entry.isDir())
            CreateDirectory(destination + "/" + entry.fileName());
        // std::cout << "FOLDER: " << QString().toStdString() << std::endl;
    }
}


/*!
 * \brief GetCurrentDirectoryPath Gets the absolute path
