...
class Simulator
{
 ...
 public:
  ...
  // -------------------------------------------------------
  /*!
   * Set the driver file path. Used when the
   ...
   * Get the fluid model.
   */
  SimulatorFluidModel fluid_model() const
  { return fluid_model_; }

  // -------------------------------------------------------
  /*!
   * Get the maximum number of minutes simulations are
   * allowed to run if no timeout value can be calculated.
   * Returns -1 if field is not set.
   */
