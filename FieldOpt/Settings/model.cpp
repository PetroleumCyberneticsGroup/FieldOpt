...
void Model::readReservoir(QJsonObject json_reservoir)
{
    ...
    if (QString::compare(type, "ECLIPSE") == 0)
        ...
        throw UnableToParseReservoirModelSectionException(
            "Grid source type " + type.toStdString()
                +  "not recognized.");
    }
...
Model::Well Model::readSingleWell(QJsonObject json_well)
{
        ...
        for (int i = 0; i < json_well_blocks.size(); ++i) {
            ...
            if (json_well_block_i.contains("Completion")) {
                ...
                if (json_well_block_i_completion.contains("IsVariable")) {
                    block.completion.is_variable =
                        json_well_block_i_completion["IsVariable"].toBool();
                }
                else
                    block.completion.is_variable = false;

                block.completion.name = "Transmissibility#"
                    + well.name + "#" + QString::number(i);
                block.has_completion = true;
            }
            else
                block.has_completion = false;
            block.name = "WellBlock#"
                + well.name + "#" + QString::number(i);
    ...
    // WellModel
    if (json_well.contains("WellModel")) {
        QString well_model = json_well["WellModel"].toString();
        if (QString::compare(well_model, "Peaceman") == 0) {
            well.well_model = WellModel::Peaceman;
        }
        else if (QString::compare(well_model, "Projection") == 0) {
            well.well_model = WellModel::Projection;
        }
    }
    else well.well_model = WellModel::Projection;
