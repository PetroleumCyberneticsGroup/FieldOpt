...
Simulator::Simulator(QJsonObject json_simulator)
{
    ...
    if (QString::compare(type, "ECLIPSE", Qt::CaseInsensitive) == 0){
        type_ = SimulatorType::ECLIPSE;
    }
    else if (QString::compare(type, "ADGPRS", Qt::CaseInsensitive) == 0){
    ...
    if (json_simulator.contains("Commands") &&
        commands.size() > 0) {
    ...
    if (script_name_.length() == 0 && commands.size() == 0){
        throw NoSimulatorCommandsGivenException(
            "At least one simulator command or "
                "a simulator script must be given.");
    }
