...
CMA_ES::Individual::Individual(Optimization::Case *c, boost::random::mt19937 &gen, int index, Eigen::VectorXd erands_norm, double penalty_dist) {
    case_pointer_ = c;
...
void CMA_ES::handleEvaluatedCase(Case *c) {
    //cout << "This is the current value of evaluated case: " << c->objective_function_value() << endl;
    if(isImprovement(c)){
        ...
        if (VERB_OPT > 0) {
            ...
            ss.precision(6);
            ss << scientific;
            ss << "New best in population, iteration " << Printer::num2str(iteration_) << ": OFV " << c->objective_function_value();
...
Case *CMA_ES::generateRandomCase() {
    Case *new_case;
