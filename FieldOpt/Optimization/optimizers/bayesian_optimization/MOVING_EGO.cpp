...
MOVING_EGO::MOVING_EGO(Settings::Optimizer *settings,
    ...
    max_iterations_ = 10;//settings->parameters().max_generations;
    ...
    lambda_ = 20; //4+floor(3*log(n_vars_));
    mu_ = lambda_/2;
    for(int i = 1; i <= mu_; i++){
...
void MOVING_EGO::handleEvaluatedCase(Case *c) {
    ...
    if (isImprovement(c)) {
        ...
        Printer::ext_info(
                "Found new tentative best case: " + Printer::num2str(c->objective_function_value()),
                "Optimization", "EGO");
    }
}
...
void MOVING_EGO::iterate() {
    ...
    for (int i = 0; i < population_.size(); i++){
        ...
        for (int j = 0; j < n_vars_; j++){
            ...
            if ((real_xmean+cub_(j)) > boundary_variables[j] && (real_xmean-clb_(j)) < boundary_variables[j]){

            } else {
                double diff = abs(real_xmean - boundary_variables[j]);
                //cout << "Difference: " <<  diff << endl;
                cub_(j) = xmean_(j)+(diff-lb_(j))/(ub_(j)-lb_(j));
                clb_(j) = xmean_(j)-(diff-lb_(j))/(ub_(j)-lb_(j));
                //cout << "This is the cub_: " << cub_(j) << " This is the clb_: " << clb_(j) << endl;
            }
        }
    }
