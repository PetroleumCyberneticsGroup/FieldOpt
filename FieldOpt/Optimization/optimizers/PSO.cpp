...
namespace Optimization{
    namespace Optimizers{
        PSO::PSO(Settings::Optimizer *settings,
            ...
            grid_ = grid;
            wic_ = wic;
            experimental_initialization_ = settings->parameters().pso_experimental_initialization;
            ...
            CellSearchBox_ = wic_->ricasedata_->mainGrid()->m_boundingBoxes_;
            ...
            for( k = QUUID_of_variables_.begin(); k != QUUID_of_variables_.end(); ++k) {
                auto continous_variable = continous_variables_->value(k.i->t());
        ...
        Case *PSO::generateRandomCase() {
            ...
            if(experimental_initialization_ && iteration_ < 2){
                ...
                for (int k = 0; k < vector_of_property_info.size(); k++ ) {
                    ...
                    if (vector_of_modified_variables.empty()) {
                        modifyvariable temp_variable_for_well;
                        vector_of_modified_variables.push_back(temp_variable_for_well);
                    }
                    int j = 0;
                    int index_of_name;
                    while((j < vector_of_modified_variables.size()) && !name_already_appared){
                    ...
                    if (vector_of_property_info[k].prop_type == Model::Properties::ContinousProperty::PolarSpline && name_already_appared) {
                        ...
                        if (vector_of_property_info[k].prop_type == Model::Properties::ContinousProperty::PolarSpline) {
                            if (vector_of_property_info[k].polar_prop == Model::Properties::ContinousProperty::Midpoint) {
                                ...
                                if (vector_of_modified_variables[0].empty){
                                    vector_of_modified_variables[0] = temp_variable_for_well;
                                    vector_of_modified_variables[0].empty = false;
                                }
                                else {
                                    temp_variable_for_well.empty = false;
                                    vector_of_modified_variables.push_back(temp_variable_for_well);
                                }
                            }
                        }
                ...
                for( int i = 0; i < vector_of_modified_variables.size(); i++){
                    ...
                    while (not isInside){
                        ...
                        for(int k = 0; k < CellSearchBox_.size(); k++){
                            if(CellSearchBox_[k].contains(check)){
                                isInside = true;
                                break;
                            }
                        }
            ...
            if(experimental_initialization_ && iteration_ < 2){
                for (int i = 0; i < vector_of_modified_variables.size(); i++){
                    ...
                    erands(vector_of_modified_variables[i].iz) = vector_of_modified_variables[i].z;
                }
            }
