...
/*
 This class will find _one_ maximum of a quadratic function
 (specified by c_, g_ and H_) subject to some specified
 constraints.

 The constraints must be specified by whomever uses this
 function and they must be specified by editing the code
 explicitly (i.e, they cannot be set through function calls).

 The objective function and the constraints (except the
 simple/basic bounds) are put together into one set of
 equations:

 Let cl_i represent a linear constraint, and let cn_i
 represent a nonlinear one. Let n_l and m be the numbers
 of linear constraints and nonlinear constraints, respectively.
 ...
 Note!
 Because F contains both the objective functions and the
 constraints, the row-indices will start at row 1 and not
 at row 0.
 ...
 Linear
   ...
   The first 2 are used b/c the matrix hatA ( "lower <= hatA*x <= upper")
   might be very sparse. An example will illustrate the usage:
 	...
 	iAfun[0] = 1; // These two indices belongs to the element (0,1) in A
 	jAvar[0] = 1; // (namely, the value 1). Now A[0] must be set to 1.
 	A[0] = 1;

	iAfun[1] = 2; // These two indices belongs to the element (1,0) in A
    jAvar[1] = 0; // (namely, the value 5). Now A[1] must be set to 5.
 	A[1] = 5;

 	iAfun[2] = 2; // These two indices belongs to the element (1,2) in A
 	jAvar[2] = 2; // (namely, the value 6). Now A[2] must be set to 6.
 	A[2] = 6;

	iAfun[3] = 3; // These two indices belongs to the element (2,2) in A
    jAvar[3] = 2; // (namely, the value 8). Now A[3] must be set to 8.
 	A[3] = 8;

 // ---------------------------------------------------------------
 Nonlinear
   The nonlinear constraints are specified through lenG, neG, iGfun
   and jGvar. The actual G matrix is specified in the userfunc.

   The G matrix contains both the derivative of the objective
   function and the derivative of the nonlinear constraints.

   The nonlinear constraints must also be specified by the userfunc,
   and put into appropriate place in F:

   F[4]=x_1^2 + x_2^2 + x_3^2;

   Let's say that we, in addition to the linear constraints above, also
   has 1 nonlinear constraint ("lower <= x_1^2 + x_2^2 + x_3^2  <= upper").
   ...
   Where y is the number of partial derivatives of the objective functions.
    ...
    iGfun[3] = 4; // NOTE NOTE! The reason why this value is 4 is because the
                  // first row is for the objective function, then we have 3
                  // linear constraints
    jGvar[3] = 0;
...
*/
 ...
 private:
  ...
  // ----------------------------------------------------------------
  // State of the variables (whether the optimal is likely to be on
  // the boundary or not)
