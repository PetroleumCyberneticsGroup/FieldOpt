...
PseudoContBoundary2D::PseudoContBoundary2D(
  ...
  try {
    ...
    // -----------------------------------------------------
    if (abs(min_block.center().z() - max_block.center().z())
    ...
    // -----------------------------------------------------
    auto affected_vars =
        variables->GetPseudoContVertVariables(well_name_);
    affected_x_var_id_ = affected_vars[0]->id();
    affected_y_var_id_ = affected_vars[1]->id();

    // -----------------------------------------------------
  } catch (std::exception const &e) {
    cerr << "Error: Initialization of PseudoContBoundary2D "
        "constraint failed for well "
         << well_name_.toStdString() << endl;
    exit(1);
  }
}
...
// =========================================================
Eigen::VectorXd
PseudoContBoundary2D::GetLowerBounds(QList<QUuid> id_vector) const {
...
// =========================================================
Eigen::VectorXd
PseudoContBoundary2D::GetUpperBounds(QList<QUuid> id_vector) const {
...
// =========================================================
void
PseudoContBoundary2D::InitializeNormalizer(QList<Case *> cases) {
