...
void ReservoirBoundary::findCornerCells() {
  ...
  // Comment: this appears to be the correct index ordering
  // for a grid cell. Check this ordering is consistent with
  // the ordering used in:
  // /ConstraintMath/well_constraint_projections
  // /well_constraint_projections.cpp

  // If not, how do the orderings correlate?

  // TODO:
  // move the output messages from here
  // to the unit test of this function.
  ...
  std::vector<Eigen::Vector3d>
      upper_plane_left_bottom_cell_xyz =
      grid_->GetCell(imin_, jmin_, kmax_).corners();

  std::vector<Eigen::Vector3d>
      upper_plane_left_top_cell_xyz =
      grid_->GetCell(imin_, jmax_, kmax_).corners();

  // -------------------------------------------------------
  // Get cell vertex amounting to true box corner
  Eigen::Vector3d
      upper_plane_left_bottom_corner_xyz =
      upper_plane_left_bottom_cell_xyz[0];

  Eigen::Vector3d
      upper_plane_left_top_corner_xyz =
      upper_plane_left_top_cell_xyz[2];
  ...
  // Get corner cells of box
  std::vector<Eigen::Vector3d>
      upper_plane_right_bottom_cell_xyz =
      grid_->GetCell(imax_, jmin_, kmax_).corners();

  std::vector<Eigen::Vector3d>
      upper_plane_right_top_cell_xyz =
      grid_->GetCell(imax_, jmax_, kmax_).corners();

  // -------------------------------------------------------
  // Get cell vertex amounting to true box corner
  Eigen::Vector3d
      upper_plane_right_bottom_corner_xyz =
      upper_plane_right_bottom_cell_xyz[1];

  Eigen::Vector3d
      upper_plane_right_top_corner_xyz =
      upper_plane_right_top_cell_xyz[3];

  // -------------------------------------------------------
  // Print coordinates
  printCornerXYZ("upper plane, right edge, bottom corner:",
                 upper_plane_right_bottom_corner_xyz);
  printCornerXYZ("upper plane, right edge, top corner:   ",
                 upper_plane_right_top_corner_xyz);
  ...
  // Get corner cells of box
  std::vector<Eigen::Vector3d>
      upper_plane_bottom_left_cell_xyz =
      grid_->GetCell(imin_, jmin_, kmax_).corners();

  std::vector<Eigen::Vector3d>
      upper_plane_bottom_right_cell_xyz =
      grid_->GetCell(imax_, jmin_, kmax_).corners();

  // -------------------------------------------------------
  // Get cell vertex amounting to true box corner
  Eigen::Vector3d
      upper_plane_bottom_left_corner_xyz =
      upper_plane_bottom_left_cell_xyz[0];

  Eigen::Vector3d
      upper_plane_bottom_right_corner_xyz =
      upper_plane_bottom_right_cell_xyz[1];

  // -------------------------------------------------------
  // Print coordinates
  printCornerXYZ(
      "upper plane, bottom edge, left corner: ",
      upper_plane_bottom_left_corner_xyz);
  printCornerXYZ(
      "upper plane, bottom edge, right corner:",
      upper_plane_bottom_right_corner_xyz);
  ...
  // Get corner cells of box
  std::vector<Eigen::Vector3d>
      upper_plane_top_left_cell_xyz =
      grid_->GetCell(imin_, jmax_, kmax_).corners();

  std::vector<Eigen::Vector3d>
      upper_plane_top_right_cell_xyz =
      grid_->GetCell(imax_, jmax_, kmax_).corners();

  // -------------------------------------------------------
  // Get cell vertex amounting to true box corner
  Eigen::Vector3d
      upper_plane_top_left_corner_xyz =
      upper_plane_top_left_cell_xyz[2];

  Eigen::Vector3d
      upper_plane_top_right_corner_xyz =
      upper_plane_top_right_cell_xyz[3];

  // -------------------------------------------------------
  // Print coordinates
  printCornerXYZ("upper plane, top edge, left corner:    ", 
    upper_plane_top_left_corner_xyz);
  printCornerXYZ("upper plane, top edge, right corner:   ",
    upper_plane_top_right_corner_xyz);
