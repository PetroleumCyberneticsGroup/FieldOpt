...
class Optimizer : public Loggable
{
 ...
 protected:
  ...
  /*!
   * @brief Handle an incomming evaluated case. This is called at the
   * end of the SubmitEvaluatedCase method.
   * @param c
   */
  virtual void handleEvaluatedCase(Case *c) = 0;

  /*!
   * @brief Check whether the Case c is an improvement on the tentative
   * best case.
  ...
  /*!
   * @brief Check if Case c1 is better than Case c2, taking into account
   * if we're maximizing or minimizing.
   */
  bool isBetter(const Case* c1, const Case *c2) const;

  /*!
   * \brief iterate Performs an iteration, generating new cases and adding
   * them to the case_handler.
   */
 ...
 protected:
  ...
  /*!
   * \brief All cases (base case, unevaluated cases and evaluated cases)
   * passed to or generated by the optimizer.
   */
