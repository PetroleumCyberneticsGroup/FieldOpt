...
/*!
 * \brief The Optimizer class is the abstract parent class
 * for all optimizers. It is primarily designed to support
...
class Optimizer
{
 public:
  ...
  /*!
   * \brief GetTentativeBestCase Get the best case found
   * so far.
   * \return
   */
  ...
  /*!
   * \brief case_handler Get the case handler. Used by
   * the bookkeeper in the runner lib.
   */
  ...
  /*!
   * The TerminationCondition enum enumerates the reasons
   * why an optimization run is deemed finished. It is
   * returned by the IsFinished method.
   */
 ...
 protected:
   ...
   * \param base_case The base case for optimizer. Must already
  ...
  /*!
   * \brief iterate Performs an iteration, generating new
   * cases and adding them to the case_handler.
   */
