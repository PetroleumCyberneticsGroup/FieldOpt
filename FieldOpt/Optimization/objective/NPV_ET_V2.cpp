...
namespace Optimization {
    namespace Objective {
        ...
        NPV_ET_V2::NPV_ET_V2(Settings::Optimizer *settings,
            ...
            components_ = new QList<NPV_ET_V2::Component *>();
            carboncomponents_ = new QList<NPV_ET_V2::Component *>();

            for (int i = 0; i < settings->objective().NPV_sum.size(); ++i) {

                auto *comp = new NPV_ET_V2::Component();
                if (settings->objective().NPV_sum[i].property.compare(0, 4, "EXT-") == 0) {
                    comp->is_json_component = true;
                    Printer::ext_info("Adding external NPV component.", "Optimization", "NPV_ET_V2");
            ...
            for (int i = 0; i < settings->objective().NPVCarbonComponents.size(); ++i){
                auto *carboncomp = new NPV_ET_V2::Component();
            ...
            e_inj_water_treatment = settings->objective().e_inj_water_treatment;    // unit: MWh/m3 injected water
        ...
        double NPV_ET_V2::calc_iwhp (double ibhp, double itvd) const
        {
            // Assumptions:
        ...
        std::vector<double> NPV_ET_V2::calc_opt_PS (double q_required, double h_required) const
        {
            int h_idx;
        ...
        double NPV_ET_V2::calc_eff_turbine_part_load (double P_turbine) const
        {
            // Units:
        ...
        QHash<QString, double> NPV_ET_V2::calc_ET() const
        {
            ...
            for (int time_idx = 0; time_idx < time.size(); time_idx++) {
                double temp = (e_water_treatment * fwpr[time_idx] / 24) + (e_inj_water_treatment * fwir[time_idx] / 24);
        ...
        double NPV_ET_V2::calc_NPV() const
        {
            try {
                double NPV = 0;
                    ...
                    if (curr_comp->interval == "Yearly") {
                        ...
                        for (int i = 0; i < report_times.size(); i++) {
                            if (i < report_times.size() - 1 &&  (report_times[i+1] - report_times[i]) > 365) {
                                ...
                                Printer::ext_warn(ss.str(), "Optimization", "NPV_ET_V2");
                                continue;
                            }
                ...
                for (int j = 0; j < components_->size(); ++j) {
                    if (components_->at(j)->is_json_component) {
                        if (components_->at(j)->interval == "Single" || components_->at(j)->interval == "None") {
                            ...
                            Printer::ext_warn("Unable to parse external component.", "Optimization", "NPV_ET_V2");
                        }
                    }
                }
        ...
        double NPV_ET_V2::value() const {
            try {
                ...
                Printer::error("Failed to compute NPV_ET_V2. Returning 0.0");

                return 0.0;
            }
        }

        QHash<QUuid, double> NPV_ET_V2::mpso_id_ofv(const QHash<QUuid, double> &mpso_id_r_CO2) const {
            try {
                QHash<QString, double> calc_ET_results = calc_ET();
                double penalty = calc_ET_results.value("penalty");
                double cost_pump = calc_ET_results.value("cost_pump");
        ...
        double NPV_ET_V2::Component::resolveValue(Simulation::Results::Results *results) {
            return results->GetValue(property);
        }
        double NPV_ET_V2::Component::resolveValueDiscount(Simulation::Results::Results *results, double time_step) {
            int time_step_int = (int) time_step;
            return results->GetValue(property, time_step_int);
        }
        double NPV_ET_V2::Component::yearlyToMonthly(double discount_rate) {
