...
// \ingroup Core
//
// Templated version of a simple array designed for high performance
// and no overhead over C arrays in release (non-check builds).
//
// Will only work on simple types and classes that do not rely on a
// constructor or destructor (as they are not called).
//
//====================================================================
...
// The current contents (data) will be kept (as much as possible).
// Any new elements will be unassigned. Calling resize(0) is the same
// as clear().
template <typename T>
...
//--------------------------------------------------------------------
/// Clear the array, freeing all memory (if not shared) and setting
/// capacity and size to zero. If the array was in shared mode, it
/// will not be in shared mode anymore.
//--------------------------------------------------------------------
template <typename T>
void Array<T>::clear()
{
...
Array<T> Array<T>::extractElements(const Array<U>& elementIndices) const
{
