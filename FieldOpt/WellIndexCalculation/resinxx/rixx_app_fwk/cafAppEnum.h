...
class AppEnum
{
 public:
  ...
  { return EnumMapper::instance()->enumVal(m_value, text);}

  bool setFromIndex (size_t index)
  { return EnumMapper::instance()->enumVal(m_value, index); }

  // Static interface to access the properties of the enum definition

  static bool isValid(const QString& text)
  { return EnumMapper::instance()->isValid(text);}
  ...
  static QStringList uiTexts()
  { return EnumMapper::instance()->uiTexts(); }

  static T fromIndex (size_t idx)
  { T val; EnumMapper::instance()->enumVal(val, idx); return val;}

  static T fromText(const QString& text)
  { T val; EnumMapper::instance()->enumVal(val, text); return val;}

  static size_t index (T enumValue)
  { return EnumMapper::instance()->index(enumValue);}

  static QString text (T enumValue)
  { return EnumMapper::instance()->text(enumValue);}

  static QString textFromIndex (size_t idx)
  { return text(fromIndex(idx));}

  static QString uiText(T enumValue)
  { return EnumMapper::instance()->uiText(enumValue);}

  static QString uiTextFromIndex(size_t idx)
  { return uiText(fromIndex(idx)); }

 private:
  //====================================================================
  // The setup method is supposed to be specialized for each and every
  // type instantiation of this class, and is supposed to set up the
  // mapping between enum values, text and ui-text using the \m addItem
  // method. It may also set a default value using \m setDefault
  //====================================================================
  static void setUp();

  static void addItem(T enumVal,
                      const QString& text,
  ...
  /// All access methods could have been placed directly in the \class
  /// AppEnum class, but AppEnum implementation gets nicer this way.
  ///
  /// The real core of this class is the vector map member and the
