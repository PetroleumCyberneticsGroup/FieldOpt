...
// =========================================================
void StructGridInterface::cellFaceVertexIndices(
    FaceType face,
...
// =========================================================
// Return values are set to cvf::UNDEFINED_SIZE_T
// if the neighbor is in the negative area
...
// =========================================================
// Models with large absolute values for coordinate
// scalars will often end up with z-fighting due to
// numerical limits in float used by OpenGL.
...
// =========================================================
void StructGridInterface::characteristicCellSizes(
    double* iSize,
    double* jSize,
    double* kSize) const {
  ...
  if (m_characteristicCellSizeI == cvf::UNDEFINED_DOUBLE
    ...
    cellFaceVertexIndices(StructGridInterface::POS_I,
                          faceConnPosI);

    // -----------------------------------------------------
    ubyte faceConnNegI[4];
    cellFaceVertexIndices(StructGridInterface::NEG_I,
                          faceConnNegI);

    // -----------------------------------------------------
    ubyte faceConnPosJ[4];
    cellFaceVertexIndices(StructGridInterface::POS_J,
                          faceConnPosJ);

    // -----------------------------------------------------
    ubyte faceConnNegJ[4];
    cellFaceVertexIndices(StructGridInterface::NEG_J,
                          faceConnNegJ);

    // -----------------------------------------------------
    ubyte faceConnPosK[4];
    cellFaceVertexIndices(StructGridInterface::POS_K,
                          faceConnPosK);

    // -----------------------------------------------------
    ubyte faceConnNegK[4];
    cellFaceVertexIndices(StructGridInterface::NEG_K,
                          faceConnNegK);
    ...
    for (k = 0; k < cellCountK(); k++) {
      ...
      for (j = 0; j < cellCountJ(); j++) {
        ...
        size_t i; // NB! Evaluate every n-th cell
        for (i = 0; i < cellCountI(); i += 10) {
