...
void RigFemPartGrid::generateStructGridData() {
  ...
  //[X]    record the ones with 3 or fewer neighbors
  //       as possible grid corners
  ...
  //[X]    by Determining what surfs correspond to NEG IJK surfaces
  //       in that element, and that none of those faces have a neighbor

  //[X] 4. Assign IJK = 000 to that element
  //[X]    Store IJK in elm idx array

  //[X] 5. Loop along POS I surfaces increment I for each element
  //       and assign IJK
  //[X]    when at end, go to POS J neighbor, increment J, repeat above.
  //[X]    etc for POS Z
  //[X]    Find max IJK as you go,
  //[ ]    also assert that there are no NEG I/NEG J/NEG Z neighbors
  ...
  // -------------------------------------------------------------
  // Debug. When we have run enough tests, remove
  CVF_ASSERT (elmIdxForIJK_000 != -1);
  ...
  // assign ijk to cells
  {
    m_ijkPrElement.resize(m_femPart->elementCount(),
                          cvf::Vec3i(-1,-1,-1));
    ...
    while (true) {
      ...
      cvf::Vec3f startElmInKNormalJ =
          m_femPart->faceNormal(elmIdxInJ, posJFaceIdx);

      cvf::Vec3f startElmInKNormalI =
          m_femPart->faceNormal(elmIdxInJ, posIFaceIdx);
      ...
      while (true) {
        ...
        cvf::Vec3f startElmInJNormalI =
            m_femPart->faceNormal(elmIdxInI, posIFaceIdx);
        int iCoord = 0;
        ...
        while (true) {
          ...
          m_ijkPrElement[elmIdxInI] = cvf::Vec3i(iCoord,
                                                 jCoord,
                                                 kCoord);
          ...
          int neighborElmIdx = m_femPart->elementNeighbor(elmIdxInI,
                                                          posIFaceIdx);
          if (neighborElmIdx == -1) break;

          // -----------------------------------------------------
          // Find the continuing face in the neighbor element
          // (opposite of the neighbor face)
          int neighborNegFaceIdx = m_femPart->neighborFace(elmIdxInI,
                                                           posIFaceIdx);
        ...
        {
          // -----------------------------------------------------
          if (iCoord > static_cast<int>(m_elmentIJKCounts[0])) {
            m_elmentIJKCounts[0] = iCoord;
          }
          ...
          // Find the continuing face in the neighbor element
          // (opposite of the neighbor face)
          int neighborNegFaceIdx =
              m_femPart->neighborFace(elmIdxInJ, posJFaceIdx);
          ...
          posIFaceIdx =
              perpendicularFaceInDirection(startElmInJNormalI,
                                           neighborNegFaceIdx,
                                           neighborElmIdx);
      ...
      {
        // -------------------------------------------------------
        if (jCoord > static_cast<int>(m_elmentIJKCounts[1])) {
          m_elmentIJKCounts[1] = jCoord;
        }
        ...
        posJFaceIdx = perpendicularFaceInDirection(startElmInKNormalJ,
                                                   neighborNegFaceIdx,
                                                   neighborElmIdx);

        posIFaceIdx = perpendicularFaceInDirection(startElmInKNormalI,
                                                   neighborNegFaceIdx,
                                                   neighborElmIdx);
    ...
    // -----------------------------------------------------------
    if (kCoord > static_cast<int>(m_elmentIJKCounts[2])) {
      m_elmentIJKCounts[2] = kCoord;
    }
  }


  // -------------------------------------------------------------
  m_elmIdxPrIJK.resize(m_elmentIJKCounts[0],
                       m_elmentIJKCounts[1],
                       m_elmentIJKCounts[2]);
...
int RigFemPartGrid::findElmIdxForIJK000() {
  ...
  const std::vector<int>&
      possibleGridCorners = m_femPart->possibleGridCornerElements();
...
int
RigFemPartGrid::perpendicularFaceInDirection(cvf::Vec3f direction,
                                             int perpFaceIdx,
                                             int elmIdx) {
...
// ===============================================================
bool RigFemPartGrid::cellIJKNeighbor(size_t i, size_t j, size_t k,
                                     FaceType face,
...
// ===============================================================
bool
RigFemPartGrid::cellIJKFromCoordinate(const cvf::Vec3d& coord,
                                      size_t* i, size_t* j, size_t* k) const {
  CVF_ASSERT(false);
  return false;
}
...
// ===============================================================
void RigFemPartGrid::cellCornerVertices(size_t cellIndex,
