...
class RigFemPart : public cvf::Object
{
 public:
  ...
  void appendElement(RigElementType elmType,
                     int elementId,
  ...
  // -------------------------------------------------------------
  size_t elementNodeResultIdx(int elementIdx,
 ...
 private:
  ...
  // Needs a more memory-friendly structure
  std::vector<std::vector<int> > m_nodeToElmRefs;

  // -------------------------------------------------------------
  void calculateElmNeighbors();

  struct Neighbors {
    int indicesToNeighborElms[6]; char faceInNeighborElm[6];
  };
