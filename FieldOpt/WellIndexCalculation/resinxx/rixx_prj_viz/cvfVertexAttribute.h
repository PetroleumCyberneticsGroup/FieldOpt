...
// ---------------------------------------------------------------
#include <resinxx/rixx_core_geom/cvfObject.h>
#include <resinxx/rixx_core_geom/cvfColor3.h>
#include "resinxx/rixx_core_geom/cvfVector4.h"
#include "resinxx/rixx_core_geom/cvfArray.h"
#include "resinxx/rixx_core_geom/cvfCharArray.h"
...
class VertexAttribute : public cvf::Object
{
 public:
  // -------------------------------------------------------------
 ...
 public:
  ...
  // Number of components per attribute element.
  virtual uint componentCount() const = 0;

  // -------------------------------------------------------------
  // The data type of the components stored in the data array
  virtual ComponentType componentType() const = 0;

  // The size of the entire data array in bytes
  virtual size_t arrayDataByteCount() const = 0;

  // Void pointer to data in contained array
  virtual const void* arrayDataPtrVoid() const = 0;

  // -------------------------------------------------------------
  // virtual void
  // setupAttribPointerBufferObject(OpenGLContext* oglContext,
  //                                uint vertexAttributeIndex,
  //                                size_t strideInBytes,
  //                                size_t bufferOffsetInBytes) const = 0;

  // -------------------------------------------------------------
  // virtual void
  // setupAttribPointerClientMemory(OpenGLContext* oglContext,
  //                                uint vertexAttributeIndex) const = 0;
};
...
class AttribSetupStrategyNormFloat
{
 public:
  // -------------------------------------------------------------
  // static void
  // setupFromBufferObject(OpenGLContext* oglContext,
  //                       uint vertexAttributeIndex,
  //                       uint compCount,
  //                       cvfGLenum compTypeOpenGL,
  //                       size_t strideInBytes,
  //                       size_t bufferOffsetInBytes);

  // -------------------------------------------------------------
  // static void setupFromClientMemory(OpenGLContext* oglContext,
  //                                   uint vertexAttributeIndex,
  //                                   uint compCount,
  //                                   cvfGLenum compTypeOpenGL,
  //                                   const void* ptr);
...
class AttribSetupStrategyDirectFloat
{
 public:
  // -------------------------------------------------------------
  // static void setupFromBufferObject(OpenGLContext* oglContext,
  //                                   uint vertexAttributeIndex,
  //                                   uint compCount,
  //                                   cvfGLenum compTypeOpenGL,
  //                                   size_t strideInBytes,
...
class VertexAttributeImpl : public VertexAttribute
{
 public:
  // -------------------------------------------------------------
  VertexAttributeImpl(const char* attribName,
                      Array<T_ELEM_TYPE>* valueArray)
  ...
  virtual uint componentCount() const
  { return T_COMP_COUNT; }

  virtual ComponentType componentType() const
  { return T_COMP_TYPE; }

  virtual size_t arrayDataByteCount() const
  { return m_attribValues->size()*sizeof(T_ELEM_TYPE); }

  virtual const void* arrayDataPtrVoid() const
  { return m_attribValues->ptr(); }

  const Array<T_ELEM_TYPE>* arrayPtr() const
  { return m_attribValues.p(); }
  ...
  setupAttribPointerBufferObject(OpenGLContext* oglContext,
                                 uint vertexAttributeIndex,
                                 size_t strideInBytes,
                                 size_t bufferOffsetInBytes) const {

    T_SETUP_STRATEGY::setupFromBufferObject(oglContext,
  ...
  virtual void
  setupAttribPointerClientMemory(OpenGLContext* oglContext,
                                 uint vertexAttributeIndex) const {

    T_SETUP_STRATEGY::setupFromClientMemory(oglContext,
                                            vertexAttributeIndex,
                                            T_COMP_COUNT,
                                            T_COMP_TYPE,
                                            m_attribValues->ptr());
 ...
 private:
  ...
  ref<Array<T_ELEM_TYPE> > m_attribValues;
