...
class DrawableGeo : public cvf::Drawable
{
 public:
  // ---------------------------------------------------------------
  enum RenderMode {
    VERTEX_ARRAY, // Rendering using client vertex arrays (copy
    // arrays from client memory to graphics memory on each draw)
    BUFFER_OBJECT // Rendering using buffer objects (arrays stored
    // in graphics memory)
  };
 ...
 public:
  ...
  // ---------------------------------------------------------------
  // virtual void render(OpenGLContext* oglContext,
  //                     ShaderProgram* shaderProgram,
  //                    const MatrixState& matrixState);

  // ---------------------------------------------------------------
  // virtual void renderFixedFunction(OpenGLContext* oglContext,
  //                                 const MatrixState& matrixState);

  // ---------------------------------------------------------------
  // virtual void renderImmediateMode(OpenGLContext* oglContext,
  //                                  const MatrixState& matrixState);
  ...
  // ---------------------------------------------------------------
  virtual bool rayIntersectCreateDetail(const Ray& ray,
                                        Vec3d* intersectionPoint,
                                        ref<HitDetail>* hitDetail) const;

 private:
  // ---------------------------------------------------------------
  // Contains all the per-vertex data, both fixed attributes such as
  // vertices and normals and generic attributes. Bundle always exists.
  ref<VertexBundle> m_vertexBundle;

  // Collection of primitive sets with the connectivity indices
  Collection<PrimitiveSet> m_primitiveSets;

  // Computed bounding box
  BoundingBox m_boundingBox;

  // Render using client memory or buffer objects?
  RenderMode m_renderMode;
};

// ===============================================================
// Base class for providing details from ray intersections with drawables
class HitDetail : public cvf::Object {
 public:
  // ---------------------------------------------------------------
  HitDetail();
  virtual ~HitDetail();
