...
namespace WellIndexCalculator{
    ...
    QList<Eigen::Vector3d> WellConstraintProjections::interwell_constraint_projection(QList<Eigen::Vector3d> coords,
                                                                                      double d)
    {
        ...
        for (int ii=0; ii<4; ii++){
            moved_coords = coords;
            temp_coords = WellConstraintProjections::well_length_projection(coords.at(two_point_index[ii][0]),
                                                                            coords.at(two_point_index[ii][1]), INFINITY, d,
                                                                            10e-5);
            moved_coords.replace(two_point_index[ii][0],temp_coords.at(0));
            moved_coords.replace(two_point_index[ii][1],temp_coords.at(1));
            if(WellConstraintProjections::shortest_distance(moved_coords) >=d &&
               WellConstraintProjections::movement_cost(coords, moved_coords) < cost){
                // If several moves of two points work, save the one with lovest movement cost
                cost = WellConstraintProjections::movement_cost(coords, moved_coords);
        ...
        for (int ii=0; ii<4; ii++){
            ...
            Eigen::Matrix3d temp_A = WellConstraintProjections::build_A_3p(input_cords_3p);
            Eigen::Vector3d temp_b = WellConstraintProjections::build_b_3p(input_cords_3p, d);
             ...
             * all of them.
             */
            QList<Eigen::Vector3d> solution_candidates = WellConstraintProjections::kkt_eq_solutions(temp_A, temp_b);
            ...
            for(int sol_num = 0; sol_num < solution_candidates.length(); sol_num++){
                ...
                std::cout << "movement cost = " << WellConstraintProjections::movement_cost(coords,moved_coords) << std::endl;*/

                if(WellConstraintProjections::shortest_distance(moved_coords) >=d-0.001 &&
                   WellConstraintProjections::movement_cost(coords, moved_coords) < cost){
                    // If several moves of two points work, save the one with lovest movement cost
                    cost = WellConstraintProjections::movement_cost(coords, moved_coords);
                    solution_coords = moved_coords;
         ...
         * all of them.
         */
        Eigen::Matrix3d temp_A = WellConstraintProjections::build_A_4p(coords);
        Eigen::Vector3d temp_b = WellConstraintProjections::build_b_4p(coords, d);
        QList<Eigen::Vector3d> solution_candidates = WellConstraintProjections::kkt_eq_solutions(temp_A, temp_b);
    ...
    double WellConstraintProjections::shortest_distance_n_wells(QList<QList<Eigen::Vector3d>> coords, int n)
    {
        ...
        for (int i=0; i<n; i++){
            for (int j=i+1; j<n; j++){
                ...
                if(WellConstraintProjections::shortest_distance(current_pair)<shortest_distance){
                    shortest_distance = WellConstraintProjections::shortest_distance(current_pair);
                }
            }
        }
        ...
        while(shortest_distance<d-tol &&iter < max_iter){
            ...
            for (int i=0; i<n; i++){
                for (int j=i+1; j<n; j++){
                    ...
                    current_pair = WellConstraintProjections::interwell_constraint_projection(current_pair, d);
    ...
    QList<QList<Eigen::Vector3d> > WellConstraintProjections::well_length_constraint_multiple_wells(QList<QList<Eigen::Vector3d> > wells, double max, double min, double epsilon)
    {
        ...
        for (int i=0; i<n; i++){
            ...
            projected_wells.append(
                    WellConstraintProjections::well_length_projection(current_heel, current_toe, max, min, epsilon));
        }
    ...
    Eigen::Vector3d WellConstraintProjections::well_domain_constraint(Eigen::Vector3d point, QList<Reservoir::Grid::Cell> cells)
    {
        double minimum = INFINITY;
    ...
    Eigen::Vector3d WellConstraintProjections::well_domain_constraint_vector(Eigen::Vector3d point, std::vector<Reservoir::Grid::Cell> cells)
    {
        double minimum = INFINITY;
        Eigen::Vector3d best_point;
    ...
    Eigen::Vector3d WellConstraintProjections::well_domain_constraint_indices(Eigen::Vector3d point, Reservoir::Grid::Grid *grid, QList<int> index_list)
    {
    ...
    double WellConstraintProjections::shortest_distance(QList<Eigen::Vector3d> coords)
    {
        /* Function runs through all possible combinations of
         * where the two closest points could be located. Return
         * when a solution is found. This function is a slightly
    ...
    double WellConstraintProjections::shortest_distance_3p(QList<Eigen::Vector3d> coords)
    {
        QList<Eigen::Vector3d> temp_coords;
    ...
    QList<Eigen::Vector3d> WellConstraintProjections::well_length_projection(Eigen::Vector3d heel, Eigen::Vector3d toe,
    ...
    Eigen::Matrix3d WellConstraintProjections::build_A_4p(QList<Eigen::Vector3d> coords)
    {
    ...
    Eigen::Vector3d WellConstraintProjections::build_b_4p(QList<Eigen::Vector3d> coords, double d)
    {
        Eigen::Vector3d vec1 = coords.at(0);
        Eigen::Vector3d vec2 = coords.at(1);
        Eigen::Vector3d vec3 = coords.at(2);
    ...
    Eigen::Matrix3d WellConstraintProjections::build_A_3p(QList<Eigen::Vector3d> coords)
    {
        Eigen::Vector3d vec1 = coords.at(0);
        Eigen::Vector3d vec2 = coords.at(1);
        Eigen::Vector3d vec3 = coords.at(2);
    ...
    Eigen::Vector3d WellConstraintProjections::build_b_3p(QList<Eigen::Vector3d> coords, double d)
    {
        Eigen::Vector3d vec1 = coords.at(0);
        Eigen::Vector3d vec2 = coords.at(1);
        Eigen::Vector3d vec3 = coords.at(2);
    ...
    Eigen::VectorXd WellConstraintProjections::coeff_vector(Eigen::Vector3d D, Eigen::Matrix3d Qinv, Eigen::Vector3d b)
    {
        double D1 = D(0);
        double D2 = D(1);
        double D3 = D(2);
    ...
    double WellConstraintProjections::movement_cost(QList<Eigen::Vector3d> old_coords, QList<Eigen::Vector3d> new_coords)
    {
    ...
    QList<Eigen::Vector3d> WellConstraintProjections::move_points_4p(QList<Eigen::Vector3d> coords, double d,
                                                                     Eigen::Vector3d s)
    {
        ...
        Eigen::Vector3d x0moved = WellConstraintProjections::project_point_to_plane(coords.at(0), s, top_plane_point);
        Eigen::Vector3d x1moved = WellConstraintProjections::project_point_to_plane(coords.at(1), s, top_plane_point);
        Eigen::Vector3d x2moved = WellConstraintProjections::project_point_to_plane(coords.at(2), s, bot_plane_point);
        Eigen::Vector3d x3moved = WellConstraintProjections::project_point_to_plane(coords.at(3), s, bot_plane_point);
    ...
    QList<Eigen::Vector3d> WellConstraintProjections::move_points_3p(QList<Eigen::Vector3d> coords, double d,
                                                                     Eigen::Vector3d s)
    {
        // Normalize s in case it is not of unit length
        s.normalize();
        ...
        Eigen::Vector3d x0moved = WellConstraintProjections::project_point_to_plane(coords.at(0), s, top_plane_point);
        Eigen::Vector3d x1moved = WellConstraintProjections::project_point_to_plane(coords.at(1), s, bot_plane_point);
        Eigen::Vector3d x2moved = WellConstraintProjections::project_point_to_plane(coords.at(2), s, bot_plane_point);
    ...
    Eigen::Vector3d WellConstraintProjections::project_point_to_plane(Eigen::Vector3d point, Eigen::Vector3d normal_vector,
    ...
    QList<Eigen::Vector3d> WellConstraintProjections::kkt_eq_solutions(Eigen::Matrix3d A, Eigen::Vector3d b)
    {
        ...
        Eigen::VectorXd coeffs = WellConstraintProjections::coeff_vector(eigenvalues, A_es.eigenvectors().inverse(), b);
