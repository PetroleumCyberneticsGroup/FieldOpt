...
QVector3D GeometryFunctions::XYZ_to_qvec(Reservoir::Grid::XYZCoordinate coor)
{
    QVector3D vec = QVector3D(coor.x(), coor.y(), coor.z());
    return vec;
}

Reservoir::Grid::XYZCoordinate GeometryFunctions::qvec_to_XYZ(QVector3D vec)
{
    QVector3D vector = vec;
    Reservoir::Grid::XYZCoordinate xyz = Reservoir::Grid::XYZCoordinate(vector.x(),vector.y(),vector.z());
    return xyz;
}
