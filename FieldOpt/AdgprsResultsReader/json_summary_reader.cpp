...
namespace AdgprsResultsReader {
    ...
    JsonSummaryReader::~JsonSummaryReader()
    {
        time_->clear();
        delete time_;
        for (auto v : field_properties_.values()) {
            delete v;
        }
    ...
    void JsonSummaryReader::readFieldProps(QJsonObject field_props)
    {
        ...
        for (QString key : keys) {
            ...
            for (QJsonValue v : field_props[key].toArray()) {
    ...
    void JsonSummaryReader::readWellProps(QJsonArray well_props)
    {
        for (QJsonValue w : well_props) {
            ...
            for (QString key : keys) {
                ...
                for (QJsonValue v : w.toObject()["Properties"].toObject()[key].toArray()) {
