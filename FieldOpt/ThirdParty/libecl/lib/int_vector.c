...
/**
   ...
   is charactereized by the following:
    ...
    o The implementation is in terms of a int, the following sed
      one-liners will then produce proper source and header files:

        sed -e's/int/int/g' vector_template.c > int_vector.c
        sed -e's/int/int/g' vector_template.h > int_vector.h
...
UTIL_SAFE_CAST_FUNCTION(int_vector , TYPE_VECTOR_ID);
UTIL_IS_INSTANCE_FUNCTION(int_vector , TYPE_VECTOR_ID);


static void int_vector_realloc_data__(int_vector_type * vector , int new_alloc_size) {
  if (new_alloc_size != vector->alloc_size) {
    if (vector->data_owner) {
      if (new_alloc_size > 0) {
        int i;
        vector->data = (int*)util_realloc(vector->data , new_alloc_size * sizeof * vector->data );
...
static void int_vector_memmove(int_vector_type * vector , int offset , int shift) {
  if ((shift + offset) < 0)
    util_abort("%s: offset:%d  left_shift:%d - invalid \n",__func__ , offset , -shift);

  if ((shift + vector->size > vector->alloc_size))
    int_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ));

  {
    size_t move_size   = (vector->size - offset) * sizeof(int);
    int * target    = &vector->data[offset + shift];
    const int * src = &vector->data[offset];
...
static void int_vector_assert_index(const int_vector_type * vector , int index) {
...
static int_vector_type * int_vector_alloc__(int init_size , int default_value, int * data, int alloc_size , bool data_owner ) {
  int_vector_type * vector = (int_vector_type*)util_malloc( sizeof * vector );
...
int_vector_type * int_vector_alloc(int init_size , int default_value) {
  int_vector_type * vector = int_vector_alloc__( init_size , default_value , NULL , 0 , true );
  return vector;
}
...
static void int_vector_assert_writable( const int_vector_type * vector ) {
  if (vector->read_only)
...
int_vector_type * int_vector_alloc_shared_wrapper(int init_size, int default_value , int * data , int alloc_size) {
  return int_vector_alloc__( init_size , default_value , data , alloc_size , false );
}
...
int int_vector_get_default(const int_vector_type * vector) {
...
int int_vector_iget(const int_vector_type * vector , int index) {
  int_vector_assert_index(vector , index);
...
int int_vector_reverse_iget(const int_vector_type * vector , int index) {
  return int_vector_iget( vector , index + vector->size );
}
...
int int_vector_safe_iget(const int_vector_type * vector, int index) {
...
int int_vector_get_last(const int_vector_type * vector) {
  return int_vector_iget(vector , vector->size - 1);
}


/** Will abort is size == 0 */
int int_vector_get_first(const int_vector_type * vector) {
  return int_vector_iget(vector , 0);
}
...
void int_vector_iset(int_vector_type * vector , int index , int value) {
  int_vector_assert_writable( vector );
  {
    if (index < 0)
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        int_vector_realloc_data__(vector , 2 * (index + 1));  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void int_vector_idel_block( int_vector_type * vector , int index , int block_size) {
  int_vector_assert_writable( vector );
  {
    if ((index >= 0) && (index < vector->size) && (block_size >= 0)) {
      ...
      index += block_size;
      int_vector_memmove( vector , index , -block_size );
    } else
...
int int_vector_idel( int_vector_type * vector , int index) {
  int del_value = int_vector_iget( vector , index );
  int_vector_idel_block( vector , index , 1 );
  return del_value;
}
...
int int_vector_del_value( int_vector_type * vector , int del_value) {
  ...
  while (true) {
    ...
    if (int_vector_iget( vector , index) == del_value) {
      int_vector_idel( vector , index);
      del_count++;
    } else
      index++;
  }
...
void int_vector_reset(int_vector_type * vector) {
  int_vector_assert_writable( vector );
  vector->size = 0;
}
...
void int_vector_free(int_vector_type * vector) {
  if (vector->data_owner)
    util_safe_free( vector->data );
  int_vector_free_container( vector );
}


void int_vector_free__(void * __vector) {
  int_vector_type * vector = (int_vector_type *) __vector;
...
int int_vector_size(const int_vector_type * vector) {
  return vector->size;
}
...
void int_vector_rshift(int_vector_type * vector , int shift) {
  if (shift < 0)
    int_vector_memmove( vector , -shift , shift);
  else {
    int i;
    int_vector_memmove( vector , 0 , shift);
    for (i=0; i < shift; i++)
      vector->data[i] = vector->default_value;
  }
}
...
void int_vector_lshift(int_vector_type * vector , int shift) {
  int_vector_rshift( vector , -shift);
}


int * int_vector_get_ptr(const int_vector_type * vector) {
  int_vector_assert_writable( vector );
  return vector->data;
}
...
const int * int_vector_get_const_ptr(const int_vector_type * vector) {
  return vector->data;
}
...
/**
   ...
   former functions will always return valid (int *) pointer,
...
int * int_vector_alloc_data_copy( const int_vector_type * vector ) {
  ...
  int * copy = (int*)util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
void int_vector_set_many(int_vector_type * vector , int index , const int * data , int length) {
  int_vector_assert_writable( vector );
  {
    int min_size = index + length;
    if (min_size > vector->alloc_size)
      int_vector_realloc_data__(vector , 2 * min_size);
...
void int_vector_append_vector(int_vector_type * vector , const int_vector_type * other) {
  int_vector_append_many( vector , int_vector_get_const_ptr( other ), int_vector_size( other ));
}
...
void int_vector_shrink(int_vector_type * vector) {
  int_vector_realloc_data__(vector , vector->size);
}
...
int int_vector_get_max(const int_vector_type * vector) {
  int max_index = int_vector_get_max_index( vector , false );
...
int int_vector_get_min(const int_vector_type * vector) {
  int min_index = int_vector_get_min_index( vector , false );
...
int int_vector_sum(const int_vector_type * vector) {
  int i;
  int sum = 0;
  for (i=0; i < vector->size; i++)
    sum += vector->data[i];
...
/**
   ...
   int_vector_index_sorted() instead.
*/
...
bool int_vector_contains(const int_vector_type * vector , int value) {
  if (int_vector_index( vector , value) >= 0)
    return true;
  else
    return false;
}
...
bool int_vector_contains_sorted(const int_vector_type * vector , int value) {
  if (int_vector_index_sorted( vector , value) >= 0)
    return true;
  else
    return false;
}
...

    {
      ...
      while (true) {
...

        {
          ...
          int center_value = vector->data[ center_index ];
...
static int int_vector_cmp(const void *_a, const void *_b) {
  int a = *((int *) _a);
  int b = *((int *) _b);
...
static int int_vector_rcmp(const void *a, const void *b) {
  return int_vector_cmp( b , a );
}
...
void int_vector_sort(int_vector_type * vector) {
  int_vector_assert_writable( vector );
  qsort(vector->data , vector->size , sizeof * vector->data ,  int_vector_cmp);
}


void int_vector_rsort(int_vector_type * vector) {
  int_vector_assert_writable( vector );
  qsort(vector->data , vector->size , sizeof * vector->data ,  int_vector_rcmp);
}



static int int_vector_cmp_node(const void *_a, const void *_b) {
...
static int int_vector_rcmp_node(const void *a, const void *b) {
  return int_vector_cmp_node( b , a );
}
...
static perm_vector_type * int_vector_alloc_sort_perm__(const int_vector_type * vector, bool reverse) {
  ...
  if (reverse)
    qsort(sort_nodes , vector->size , sizeof * sort_nodes ,  int_vector_rcmp_node);
  else
    qsort(sort_nodes , vector->size , sizeof * sort_nodes ,  int_vector_cmp_node);
...
perm_vector_type * int_vector_alloc_sort_perm(const int_vector_type * vector) {
  return int_vector_alloc_sort_perm__( vector , false );
}


perm_vector_type *  int_vector_alloc_rsort_perm(const int_vector_type * vector) {
  return int_vector_alloc_sort_perm__( vector , true );
}
...
void int_vector_permute(int_vector_type * vector , const perm_vector_type * perm) {
  ...
  {
    int i;
    int * tmp = (int*)util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)
...
int int_vector_lookup_bin( const int_vector_type * limits , int value , int guess) {
  ...
  return int_vector_lookup_bin__( limits , value , guess );
}
...
/*
  ...
  and that guess < limits->size. See int_vector_lookup_bin() for
     ...
     binary search. */
  {
    ...
    while (true) {
...

      {
        ...
        int central_value = limits->data[ central_index ];
...
void int_vector_fwrite(const int_vector_type * vector , FILE * stream) {
  ...
  int_vector_fwrite_data( vector , stream );
}
...
int_vector_type * int_vector_buffer_fread_alloc( buffer_type * buffer ) {
  int_vector_type * vector = int_vector_alloc( 0 , 0);
  int_vector_buffer_fread( vector , buffer );
  return vector;
}
