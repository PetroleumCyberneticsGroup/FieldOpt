...
/**
   ...
   is charactereized by the following:
    ...
    o The implementation is in terms of a size_t, the following sed
      one-liners will then produce proper source and header files:

        sed -e's/size_t/int/g' vector_template.c > int_vector.c
        sed -e's/size_t/int/g' vector_template.h > int_vector.h
...
static const char * string_type = "size_t";
...
UTIL_SAFE_CAST_FUNCTION(size_t_vector , TYPE_VECTOR_ID);
UTIL_IS_INSTANCE_FUNCTION(size_t_vector , TYPE_VECTOR_ID);


static void size_t_vector_realloc_data__(size_t_vector_type * vector , int new_alloc_size) {
  if (new_alloc_size != vector->alloc_size) {
    if (vector->data_owner) {
      if (new_alloc_size > 0) {
        int i;
        vector->data = (size_t*)util_realloc(vector->data , new_alloc_size * sizeof * vector->data );
...
static void size_t_vector_memmove(size_t_vector_type * vector , int offset , int shift) {
  if ((shift + offset) < 0)
...
static void size_t_vector_assert_index(const size_t_vector_type * vector , int index) {
...
static size_t_vector_type * size_t_vector_alloc__(int init_size , size_t default_value, size_t * data, int alloc_size , bool data_owner ) {
  size_t_vector_type * vector = (size_t_vector_type*)util_malloc( sizeof * vector );
  ...
  */
  ...
  size_t_vector_set_read_only( vector , false );
  if (init_size > 0)
    size_t_vector_iset( vector , init_size - 1 , default_value );  /* Filling up the init size elements with the default value */
...
size_t_vector_type * size_t_vector_alloc(int init_size , size_t default_value) {
  size_t_vector_type * vector = size_t_vector_alloc__( init_size , default_value , NULL , 0 , true );
  return vector;
}
...
void size_t_vector_resize( size_t_vector_type * vector , int new_size ) {
  if (new_size <= vector->size)
    vector->size = new_size;
  else
    size_t_vector_iset( vector , new_size - 1 , vector->default_value);
}
...
static void size_t_vector_assert_writable( const size_t_vector_type * vector ) {
  if (vector->read_only)
...
void size_t_vector_memcpy_data_block( size_t_vector_type * target , const size_t_vector_type * src , int target_offset , int src_offset , int len) {
  ...
  if (target->alloc_size < (target_offset + len))
    size_t_vector_iset( target , target_offset + len - 1 , target->default_value) ;
...
void size_t_vector_memcpy_from_data( size_t_vector_type * target , const size_t * src , int src_size ) {
  size_t_vector_reset( target );
  size_t_vector_iset( target , src_size - 1 , 0 );
...
void size_t_vector_memcpy( size_t_vector_type * target, const size_t_vector_type * src ) {
  ...
  size_t_vector_memcpy_data_block( target  , src , 0 , 0 , src->size );
}
...
size_t_vector_type * size_t_vector_alloc_strided_copy( const size_t_vector_type * src , int start , int stop , int stride ) {
  size_t_vector_type * copy = size_t_vector_alloc( 0 , src->default_value );
  if (start < 0)
...

  {
    ...
    while (src_index < stop) {
      size_t_vector_append( copy , size_t_vector_iget( src , src_index ));
      src_index += stride;
    }
  }
  return copy;
...
size_t_vector_type * size_t_vector_alloc_copy( const size_t_vector_type * src) {
  size_t_vector_type * copy = size_t_vector_alloc( src->size , src->default_value );
  size_t_vector_realloc_data__( copy , src->alloc_size );
  copy->size = src->size;
...
size_t size_t_vector_get_default(const size_t_vector_type * vector) {
...
void size_t_vector_append_default(size_t_vector_type * vector , size_t default_value) {
  size_t_vector_append( vector , default_value );
  size_t_vector_set_default( vector , default_value );
}
...
void size_t_vector_iset_default(size_t_vector_type * vector , int index , size_t default_value) {
  size_t_vector_iset( vector , index , default_value );
  size_t_vector_set_default( vector , default_value );
}
...
size_t size_t_vector_iget(const size_t_vector_type * vector , int index) {
...
size_t size_t_vector_reverse_iget(const size_t_vector_type * vector , int index) {
  return size_t_vector_iget( vector , index + vector->size );
}
...
size_t size_t_vector_safe_iget(const size_t_vector_type * vector, int index) {
...
size_t size_t_vector_get_last(const size_t_vector_type * vector) {
  return size_t_vector_iget(vector , vector->size - 1);
}


/** Will abort is size == 0 */
size_t size_t_vector_get_first(const size_t_vector_type * vector) {
  return size_t_vector_iget(vector , 0);
}
...
void size_t_vector_iset(size_t_vector_type * vector , int index , size_t value) {
  size_t_vector_assert_writable( vector );
  {
    if (index < 0)
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        size_t_vector_realloc_data__(vector , 2 * (index + 1));  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void size_t_vector_iset_block(size_t_vector_type * vector , int index , int block_size , size_t value) {
  ...
  for (c=0; c < abs(block_size); c++)
    size_t_vector_iset( vector , index + c * sign , value);
}
...
size_t size_t_vector_iadd( size_t_vector_type * vector , int index , size_t delta) {
  size_t new_value     = size_t_vector_safe_iget(vector , index ) + delta;
  size_t_vector_iset( vector , index , new_value );
  return new_value;
}
...
size_t size_t_vector_idel( size_t_vector_type * vector , int index) {
  ...
  size_t_vector_idel_block( vector , index , 1 );
  return del_value;
}
...
size_t size_t_vector_del_value( size_t_vector_type * vector , size_t del_value) {
  ...
  while (true) {
    ...
    if (size_t_vector_iget( vector , index) == del_value) {
      size_t_vector_idel( vector , index);
      del_count++;
    } else
      index++;
  }
...
void size_t_vector_insert( size_t_vector_type * vector , int index , size_t value) {
  if (index >= vector->size)
    size_t_vector_iset( vector , index , value );
  else {
    size_t_vector_memmove( vector , index , 1 );
    size_t_vector_iset( vector , index , value );
  }
}


void size_t_vector_append(size_t_vector_type * vector , size_t value) {
  size_t_vector_iset(vector , vector->size , value);
}


void size_t_vector_reset(size_t_vector_type * vector) {
  size_t_vector_assert_writable( vector );
  vector->size = 0;
}
...
void size_t_vector_free_data(size_t_vector_type * vector) {
  size_t_vector_reset(vector);
  size_t_vector_realloc_data__(vector , 0);
}


void size_t_vector_free(size_t_vector_type * vector) {
  if (vector->data_owner)
    util_safe_free( vector->data );
  size_t_vector_free_container( vector );
}


void size_t_vector_free__(void * __vector) {
  size_t_vector_type * vector = (size_t_vector_type *) __vector;
  size_t_vector_free( vector );
}


void size_t_vector_reset__(void * __vector) {
  size_t_vector_type * vector = size_t_vector_safe_cast( __vector );
  size_t_vector_reset( vector );
}


int size_t_vector_size(const size_t_vector_type * vector) {
  return vector->size;
}
...
size_t size_t_vector_pop(size_t_vector_type * vector) {
  ...
  {
    if (vector->size > 0) {
      size_t value = vector->data[vector->size - 1];
      vector->size--;
      return value;
    } else {
...
void size_t_vector_rshift(size_t_vector_type * vector , int shift) {
  if (shift < 0)
    size_t_vector_memmove( vector , -shift , shift);
  else {
    int i;
    size_t_vector_memmove( vector , 0 , shift);
    for (i=0; i < shift; i++)
      vector->data[i] = vector->default_value;
  }
}
...
void size_t_vector_lshift(size_t_vector_type * vector , int shift) {
  size_t_vector_rshift( vector , -shift);
}


size_t * size_t_vector_get_ptr(const size_t_vector_type * vector) {
  size_t_vector_assert_writable( vector );
  return vector->data;
}
...
const size_t * size_t_vector_get_const_ptr(const size_t_vector_type * vector) {
  return vector->data;
}
...
/**
   ...
   former functions will always return valid (size_t *) pointer,
...
size_t * size_t_vector_alloc_data_copy( const size_t_vector_type * vector ) {
  int      size = vector->size * sizeof ( size_t );
  size_t * copy = (size_t*)util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
void size_t_vector_set_many(size_t_vector_type * vector , int index , const size_t * data , int length) {
  size_t_vector_assert_writable( vector );
  {
    int min_size = index + length;
    if (min_size > vector->alloc_size)
      size_t_vector_realloc_data__(vector , 2 * min_size);
...
void size_t_vector_init_range(size_t_vector_type * vector , size_t value1 , size_t value2 , size_t delta) {
  ...
  if (delta != 0) {
    ...
    if (valid_range) {
      size_t current_value = value1;
      while (true) {
	    size_t_vector_append( vector , current_value );
	    current_value += delta;
...
void size_t_vector_append_many(size_t_vector_type * vector , const size_t * data , int length) {
  size_t_vector_set_many( vector , size_t_vector_size( vector ) , data , length);
}

void size_t_vector_append_vector(size_t_vector_type * vector , const size_t_vector_type * other) {
  size_t_vector_append_many( vector , size_t_vector_get_const_ptr( other ), size_t_vector_size( other ));
}
...
void size_t_vector_shrink(size_t_vector_type * vector) {
  size_t_vector_realloc_data__(vector , vector->size);
}
...
size_t size_t_vector_get_max(const size_t_vector_type * vector) {
  int max_index = size_t_vector_get_max_index( vector , false );
...
size_t size_t_vector_get_min(const size_t_vector_type * vector) {
  int min_index = size_t_vector_get_min_index( vector , false );
...
size_t size_t_vector_sum(const size_t_vector_type * vector) {
  int i;
  size_t sum = 0;
  for (i=0; i < vector->size; i++)
    sum += vector->data[i];
...
/**
   ...
   size_t_vector_index_sorted() instead.
*/
...
bool size_t_vector_contains(const size_t_vector_type * vector , size_t value) {
  if (size_t_vector_index( vector , value) >= 0)
    return true;
  else
    return false;
}
...
bool size_t_vector_contains_sorted(const size_t_vector_type * vector , size_t value) {
  if (size_t_vector_index_sorted( vector , value) >= 0)
    return true;
  else
    return false;
}
...
static int size_t_vector_cmp(const void *_a, const void *_b) {
  size_t a = *((size_t *) _a);
  size_t b = *((size_t *) _b);
...
static int size_t_vector_rcmp(const void *a, const void *b) {
  return size_t_vector_cmp( b , a );
}
...
void size_t_vector_select_unique(size_t_vector_type * vector) {
  size_t_vector_assert_writable( vector );
  if (vector->size > 0) {
    size_t_vector_type * copy = size_t_vector_alloc_copy( vector );
    size_t_vector_sort( copy );
    size_t_vector_reset( vector );
    {
      int i;
      size_t previous_value = size_t_vector_iget( copy , 0);
      size_t_vector_append( vector , previous_value);

      for (i=1; i <  copy->size; i++) {
        size_t value = size_t_vector_iget( copy , i );
        if (value != previous_value)
          size_t_vector_append( vector , value);
        previous_value = value;
      }
    }
    size_t_vector_free( copy );
  }
}
...
void size_t_vector_sort(size_t_vector_type * vector) {
  size_t_vector_assert_writable( vector );
  qsort(vector->data , vector->size , sizeof * vector->data ,  size_t_vector_cmp);
}


void size_t_vector_rsort(size_t_vector_type * vector) {
  size_t_vector_assert_writable( vector );
  qsort(vector->data , vector->size , sizeof * vector->data ,  size_t_vector_rcmp);
}
...
static perm_vector_type * size_t_vector_alloc_sort_perm__(const size_t_vector_type * vector, bool reverse) {
...
perm_vector_type *  size_t_vector_alloc_rsort_perm(const size_t_vector_type * vector) {
  return size_t_vector_alloc_sort_perm__( vector , true );
}
...
void size_t_vector_permute(size_t_vector_type * vector , const perm_vector_type * perm) {
  ...
  {
    int i;
    size_t * tmp = (size_t*)util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)
...
int size_t_vector_lookup_bin( const size_t_vector_type * limits , size_t value , int guess) {
  ...
  return size_t_vector_lookup_bin__( limits , value , guess );
}
...
/*
  ...
  and that guess < limits->size. See size_t_vector_lookup_bin() for
     ...
     binary search. */
  {
    ...
    while (true) {
...

      {
        ...
        size_t central_value = limits->data[ central_index ];
...
void size_t_vector_fread_data( size_t_vector_type * vector , int size, FILE * stream) {
  size_t_vector_realloc_data__( vector , size );
...
void size_t_vector_fwrite(const size_t_vector_type * vector , FILE * stream) {
  ...
  size_t_vector_fwrite_data( vector , stream );
}
...
void size_t_vector_fread( size_t_vector_type * vector , FILE * stream ) {
  ...
  {
    size_t_vector_set_default( vector , default_value );
    size_t_vector_fread_data( vector , size , stream );
  }
}
...
size_t_vector_type * size_t_vector_fread_alloc( FILE * stream ) {
  size_t_vector_type * vector = size_t_vector_alloc( 0,0 );
  size_t_vector_fread( vector , stream );
  return vector;
}
...
size_t_vector_type * size_t_vector_buffer_fread_alloc( buffer_type * buffer ) {
  size_t_vector_type * vector = size_t_vector_alloc( 0 , 0);
  size_t_vector_buffer_fread( vector , buffer );
  return vector;
}
