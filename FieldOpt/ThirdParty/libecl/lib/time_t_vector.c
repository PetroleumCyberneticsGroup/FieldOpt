...
static void time_t_vector_realloc_data__(time_t_vector_type * vector , int new_alloc_size) {
...
static void time_t_vector_memmove(time_t_vector_type * vector , int offset , int shift) {
  ...
  if ((shift + vector->size > vector->alloc_size))
    time_t_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ));
...
static time_t_vector_type * time_t_vector_alloc__(int init_size , time_t default_value, time_t * data, int alloc_size , bool data_owner ) {
  ...
  */
  ...
  if (init_size > 0)
    time_t_vector_iset( vector , init_size - 1 , default_value );  /* Filling up the init size elements with the default value */
...
void time_t_vector_resize( time_t_vector_type * vector , int new_size ) {
  if (new_size <= vector->size)
    vector->size = new_size;
  else
    time_t_vector_iset( vector , new_size - 1 , vector->default_value);
}
...
time_t_vector_type * time_t_vector_alloc_copy( const time_t_vector_type * src) {
  ...
  time_t_vector_realloc_data__( copy , src->alloc_size );
  copy->size = src->size;
  ...
  time_t_vector_assert_writable( vector );
  {
      ...
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        time_t_vector_realloc_data__(vector , 2 * (index + 1));  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void time_t_vector_free_data(time_t_vector_type * vector) {
  ...
  time_t_vector_realloc_data__(vector , 0);
}
...
void time_t_vector_set_many(time_t_vector_type * vector , int index , const time_t * data , int length) {
  time_t_vector_assert_writable( vector );
  {
    ...
    if (min_size > vector->alloc_size)
      time_t_vector_realloc_data__(vector , 2 * min_size);
...
void time_t_vector_shrink(time_t_vector_type * vector) {
  time_t_vector_realloc_data__(vector , vector->size);
}
...
void time_t_vector_fread_data( time_t_vector_type * vector , int size, FILE * stream) {
  time_t_vector_realloc_data__( vector , size );
...
time_t_vector_type * time_t_vector_buffer_fread_alloc( buffer_type * buffer ) {
  time_t_vector_type * vector = time_t_vector_alloc( 0 , 0);
  time_t_vector_buffer_fread( vector , buffer );
  return vector;
}
