...
static void double_vector_realloc_data__(double_vector_type * vector , int new_alloc_size) {
...
static void double_vector_memmove(double_vector_type * vector , int offset , int shift) {
  ...
  if ((shift + vector->size > vector->alloc_size))
    double_vector_realloc_data__( vector , util_int_min( 2*vector->alloc_size , shift + vector->size ));
...
static double_vector_type * double_vector_alloc__(int init_size , double default_value, double * data, int alloc_size , bool data_owner ) {
  ...
  */
  ...
  if (init_size > 0)
    double_vector_iset( vector , init_size - 1 , default_value );  /* Filling up the init size elements with the default value */
...
void double_vector_resize( double_vector_type * vector , int new_size ) {
  if (new_size <= vector->size)
    vector->size = new_size;
  else
    double_vector_iset( vector , new_size - 1 , vector->default_value);
}
...
double_vector_type * double_vector_alloc_copy( const double_vector_type * src) {
  ...
  double_vector_realloc_data__( copy , src->alloc_size );
  copy->size = src->size;
  ...
  double_vector_assert_writable( vector );
  {
      ...
      util_abort("%s: Sorry - can NOT set negative indices. called with index:%d \n",__func__ , index);
    {
      if (vector->alloc_size <= index)
        double_vector_realloc_data__(vector , 2 * (index + 1));  /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void double_vector_free_data(double_vector_type * vector) {
  ...
  double_vector_realloc_data__(vector , 0);
}
...
void double_vector_set_many(double_vector_type * vector , int index , const double * data , int length) {
  double_vector_assert_writable( vector );
  {
    ...
    if (min_size > vector->alloc_size)
      double_vector_realloc_data__(vector , 2 * min_size);
...
void double_vector_shrink(double_vector_type * vector) {
  double_vector_realloc_data__(vector , vector->size);
}
...
void double_vector_fread_data( double_vector_type * vector , int size, FILE * stream) {
  double_vector_realloc_data__( vector , size );
...
double_vector_type * double_vector_buffer_fread_alloc( buffer_type * buffer ) {
  double_vector_type * vector = double_vector_alloc( 0 , 0);
  double_vector_buffer_fread( vector , buffer );
  return vector;
}
