...
void AbstractRunner::InitializeOptimizer()
{
    ...
    switch (settings_->optimizer()->type()) {
        ...
        case Settings::Optimizer::OptimizerType::EGO:
            ...
            optimizer_->SetVerbosityLevel(runtime_settings_->verbosity_level());
            break;
        case Settings::Optimizer::OptimizerType::BO_PSO:
            if (VERB_RUN >= 1) Printer::ext_info("Using BO_PSO optimization algorithm.", "Runner", "AbstractRunner");
            optimizer_ = new Optimization::Optimizers::BayesianOptimization::BO_PSO(settings_->optimizer(),
                                                                                 base_case_,
                                                                                 model_->variables(),
                                                                                 model_->grid(),
                                                                                 logger_
