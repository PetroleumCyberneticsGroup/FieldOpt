...
class GridTest : public ::testing::Test, TestResources::TestResourceGrids {
 protected:
  GridTest() : Test(), TestResourceGrids() {
      grid_ = grid_horzwel_;
      grid_5sp_ = grid_5spot_;
      grid_nor_ = grid_norne_;
  }
...
TEST_F(GridTest, FindSmallestCell) {
    auto smallest_horzwell = grid_->GetSmallestCell();
    auto smallest_norne = grid_nor_->GetSmallestCell();
    EXPECT_LT(smallest_norne.volume(), grid_nor_->GetCell(8, 11, 6).volume());
