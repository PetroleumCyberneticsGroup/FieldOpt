...
Simulator::Simulator(Settings::Settings *settings) {
    ...
    if (!Utilities::FileHandling::DirectoryExists(settings->include_directory())) {
        IncludeDirectoryDoesNotExistException(settings->output_directory());
    }
    include_directory_ = settings->output_directory();
