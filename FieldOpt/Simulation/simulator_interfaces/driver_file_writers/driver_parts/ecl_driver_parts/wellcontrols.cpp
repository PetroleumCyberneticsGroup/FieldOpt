...
QString WellControls::getTimestepPartString() const {
  ...
  // When this method is used, the time entries map
  // should only contain two entries: one for the current
  // timestep that includes well data, and one for the
  // next that should only include the time to be used
...
// =========================================================
int WellControls::nextTimestep(const int &timestep,
...
// =========================================================
void WellControls::initializeTimeEntries(
    const QList<Model::Wells::Well *> *wells,
    const QList<double> &control_times) {
    ...
    if (wells->at(i)->trajectory()->GetWellBlocks()->size() > 0) {
      ...
      for (int j = 0; j < wells->at(i)->controls()->size(); ++j) {
        ...
        if (time_entries_.keys().contains(current_time_step)) {
          ...
          auto extended_well_settings =
              time_entries_.value(current_time_step).well_settings;

          // -----------------------------------------------
          extended_well_settings.append(
              WellSetting(wells->at(i)->name(),
                          wells->at(i)->IsInjector(),
                          *wells->at(i)->controls()->at(j)));

          // -----------------------------------------------
          time_entries_[current_time_step].well_settings =
              extended_well_settings;
...
//OLD VERSION, KEEP FOR REF. DELETE EVENTUALLY
// =========================================================
//void WellControls::
...
// =========================================================
QString WellControls::createTimeEntry(const double &time,
...
createProducerEntry(const WellControls::WellSetting &setting) const {
...
// =========================================================
QString WellControls::
createInjectorEntry(const WellControls::WellSetting &setting) const {
