...
class ECLSimulator : public Simulator
{
 public:
  // -------------------------------------------------------
  ECLSimulator(Settings::Settings *settings,
               Model::Model *model);

  // -------------------------------------------------------
  /*!
   * \brief Evaluate Executes the simulation of the
   * current model. The evaluation is blocking.
   */
  void Evaluate();
  ...
  // -------------------------------------------------------
  /*!
   * \brief CleanUp Deletes files created during the
   * simulation. All files except the .DATA, .UNSMRY,
   * .SMSPEC  and .LOG are deleted.
   */
  void CleanUp();
 ...
 private:
  ...
  QString output_sim_incl_dir_name_;
