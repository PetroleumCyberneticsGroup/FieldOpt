...
void ECLResults::ReadResults(QString file_path, const std::vector<int> verb_vector)
{
    if (summary_reader_ != 0) delete summary_reader_;
    try {
        summary_reader_ = new ERTWrapper::ECLSummary::ECLSummaryReader(file_path.toStdString(),
...
double ECLResults::GetValue(Results::Property prop, int time_index)
{
    ...
    if (time_index < 0 || time_index >= summary_reader_->time().size())
        throw std::runtime_error("The time index " + boost::lexical_cast<std::string>(time_index)
                                     + " is outside the range of the summary.");
...
std::vector<double> ECLResults::GetValueVector(Results::Property prop)
{
    ...
    switch (prop) {
        ...
        default: throw std::runtime_error("In ECLResults: The requested property "
                                              "is not a field or misc property.");
    }
}
