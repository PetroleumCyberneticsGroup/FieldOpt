...
// =========================================================
double ECLSummaryReader::GetMiscVar(string var_name,
                                    int time_index) {
  if (!hasMiscVar(var_name)) {
    throw SummaryVariableDoesNotExistException(
        "Misc variable " + std::string(var_name) + " does not exist.");
  }
...
// =========================================================
double ECLSummaryReader::GetFieldVar(string var_name,
                                     int time_index) {

  if (!HasReportStep(time_index)) {
    throw SummaryTimeStepDoesNotExistException(
        "Time step does not exist");
  }

  if (!hasFieldVar(var_name)) {
    throw SummaryVariableDoesNotExistException(
        "Field variable" + var_name + " does not exist.");
  }
  return ecl_sum_get_field_var(ecl_sum_,
                               time_index,
                               var_name.c_str());
}

// =========================================================
double ECLSummaryReader::GetWellVar(string well_name,
                                    string var_name,
                                    int time_index) {

  if (!hasWellVar(well_name, var_name)) {
    throw SummaryVariableDoesNotExistException(
        "Well variable " + std::string(well_name) + ":"
  ...
  return ecl_sum_get_well_var(ecl_sum_,
                              time_index,
                              well_name.c_str(),
                              var_name.c_str());
}
...
bool ECLSummaryReader::HasReportStep(int report_step) {
  return report_step <= GetLastReportStep()
      && report_step >= GetFirstReportStep();
}

// =========================================================
bool ECLSummaryReader::hasWellVar(string well_name,
                                  string var_name) {
...
// =========================================================
bool ECLSummaryReader::hasGroupVar(string group_name,
                                   string var_name) {
...
// =========================================================
bool ECLSummaryReader::hasBlockVar(int block_nr,
                                   string var_name) {
...
void ECLSummaryReader::populateKeyLists() {
  ...
  // -------------------------------------------------------
  if (verb_vector_[1] > 1) // idx:1 -> ert
    cout << "ecl_sum_alloc_matching_general_var_list" << endl;
  stringlist_type * keys =
      ecl_sum_alloc_matching_general_var_list(ecl_sum_,
                                              NULL);

  // -------------------------------------------------------
  if (verb_vector_[1] > 1) // idx:1 -> ert
    cout << "ecl_sum_alloc_well_list" << endl;
  stringlist_type * wells =
      ecl_sum_alloc_well_list(ecl_sum_,
                              NULL);

  // -------------------------------------------------------
  if (verb_vector_[1] > 1) // idx:1 -> ert
    cout << "ecl_sum_alloc_matching_general_var_list" << endl;
  stringlist_type * field_keys =
      ecl_sum_alloc_matching_general_var_list(ecl_sum_,
                                              "F*");

  // -------------------------------------------------------
  if (verb_vector_[1] > 1) // idx:1 -> ert
    cout << "ecl_sum_alloc_well_var_list" << endl;
  stringlist_type * well_keys =
      ecl_sum_alloc_well_var_list(ecl_sum_);
...
void ECLSummaryReader::warnPropertyZero(string wname, string propname) const {
  std::cerr << "WARNING: Returning cumulative vector with final falue 0.0 for "
      "property " + propname + " for well " + wname + ". The vector may be "
      "uninitialized because the property is not found for the well, or because "
      "the phase does not exist in the system." << std::endl;
}

// =========================================================
void ECLSummaryReader::warnPropertyNotFound(string propname) const {
  std::cerr << "WARNING: The field property " + propname +
      " was not found in the summary. Attempting to calculate "
          "it from corresponding well property." << std::endl;
}

// =========================================================
void ECLSummaryReader::warnPropertyZero(string propname) const {
  std::cerr << "WARNING: Returning cumulative vector with final value 0.0 for "
      "property " + propname + ". The vector may be uninitialized because "
